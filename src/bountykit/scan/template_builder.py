...
def build_template(
    ...
    """Dispatch to the appropriate template builder based on vulnerability type.

    Args:
        vuln_type: Vulnerability type (sqli, xss, idor, ssrf, lfi, rce)
        param: Parameter name or path
        path: URL path
        output_dir: Output directory
    ...
    """
    builder_map = {
        "sqli": build_sqli_template,
        "xss": build_xss_template,
        "idor": build_idor_template,
    }

    builder = builder_map.get(vuln_type)
    if not builder:
        console.print(f"[bold red]Unsupported vulnerability type: {vuln_type}[/bold red]")
        console.print("[yellow]Supported types: sqli, xss, idor, ssrf, lfi, rce[/yellow]")
        return ""

    os.makedirs(output_dir, exist_ok=True)

    if vuln_type == "idor":
        paths_to_test = [path] if path else ["/api/v1/users/1"]
        return builder(path, paths_to_test, output_dir)
    else:
        params_to_test = [param] if param else ["id"]
        return builder(path, params_to_test, output_dir)
